...
app.put("/api/videos/:id", async (req, res) => {
  try {
    ...
    // Extract video ID from YouTube URL if URL changed
...
app.post("/api/volunteers", upload.single("image"), async (req, res) => {
  try {
    ...
    if (req.file) {
      ...
      // Generate avatar if no image uploaded
...
app.get("/api/events", async (req, res) => {
  try {
    ...
    // If upcoming=true, only show events from today onwards
    if (upcoming === "true") {
...
// Create QR Chanda (Public - No authentication needed)
app.post("/api/chanda/qr", async (req, res) => {
  try {
    ...
    // TRANSACTION ID IS MANDATORY for QR payments
    ...
    // Return full data including chandaId
    res.json({
      success: true,
      data: {
        _id: chanda._id,
